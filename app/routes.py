...
async def create_short_url(request: URLRequest, session: Session = Depends(get_session)):
    # check if url exists
    ...
    return create_URLResponse(
        crud.create_short_url(session, request.url, short_code)
        )
...
def update_short_url(short_code: str, request: URLRequest, session: Session = Depends(get_session)):
    entry = crud.update_short_url(session=session, url=request.url, short_code=short_code)
